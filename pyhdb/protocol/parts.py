...
class Command(Part):
    ...
    def pack_data(self, remaining_size):
        payload = self.sql_statement.encode('utf-8')
        return 1, payload

    @classmethod
    def unpack_data(cls, argument_count, payload):
        sql_statement = payload.read()
        return sql_statement.decode('utf-8')
    ...
    def unpack_rows(self, parameters_metadata, connection):
        ...
        for param in parameters_metadata:
            ...
            if param.iotype != parameter_direction.IN:
                values.append( by_type_code[param.datatype].from_resultset(self.payload, connection) )  # connection requred for LOBs .read() call
        yield tuple(values)
